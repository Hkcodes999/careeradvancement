...
const AUTOPILOT_MAP = {
  ...
  "UG": { categories: ["technical", "logical", "problemSolving"], difficulty: "medium", questionCount: 20, timePerQuestion: 90 },
  "PG": { categories: ["technical", "logical", "problemSolving", "communication"], difficulty: "hard", questionCount: 25, timePerQuestion: 90 },
};
...
async function runAutopilot(user, institutionId) {
  try {
    ...
    if (!existingAssessment) {
      ...
      const questionCount = policy?.questionsPerCategory || fallbackConfig.questionCount;
      ...
      await Assessment.create({
        ...
        timePerQuestion: policy?.timeLimit ? (policy.timeLimit * 60) / questions.length : fallbackConfig.timePerQuestion,
        questions,
        slot: batch.slot 
      });
    }
...
exports.saveStudentProfile = async (req, res) => {
  try {
    ...
    const { phone, age, gender, education, stream, city, state, skills, careerGoal, others } = req.body;

    user.profile = {
      phone, age, gender, education, stream, city, state, careerGoal, others,
    ...
    res.json({ success: true, message: "Profile saved. Autopilot initiated.", profile: user.profile });
  } catch (err) {
...
exports.selectInstitution = async (req, res) => {
  try {
    ...
    res.json({ success: true, message: "Institution selected and Autopilot triggered" });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
};